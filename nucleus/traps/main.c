...
void main() 
{
	...
	int sp = global_state.s_sp;
	sp = sp - PAGESIZE*2; // Move stack pointer down from the initial global process area
	p1_state->s_sp = sp;
	p1_state->s_pc = (int)p1;

	// Now set the status register flags for p1 process state
	p1_state->s_sr.ps_int = 7;  // All interrupts disabled for inital p1 proc
	p1_state->s_sr.ps_s = 0;	// Set memory management to physical addressing (no process virutalization)
	p1_state->s_sr.ps_m = 1;    // Switch to Supervisor Mode to run initial proc
